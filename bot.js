...
client.on("message", (message) => {
    /// NYR
   if (message.content.startsWith("+new")) {     /// NYR
        const reason = message.content.split(" ").slice(1).join(" ");     /// NYR
...
client.on('message', message => {
    ...
    .addField('شكوى على:', `${user}`)
    .addField('سبب الشكوى:', `${reason}`)
    .setFooter(client.user.username)
...
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","✱ NY"));
    });
